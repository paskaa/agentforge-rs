  ...
  <div>
    ...
    <el-card shadow="never" v-if="scores.length">
      ...
      <div v-for="(s, i) in scores" :key="s.agent_id" style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
        ...
        <el-progress :percentage="s.overall_score || 0" :stroke-width="14" style="flex:1" :color="s.overall_score > 50 ? '#22c55e' : s.overall_score > 25 ? '#f59e0b' : '#ef4444'" />
        <span style="width:40px;text-align:right;font-weight:600;font-size:13px">{{ s.overall_score?.toFixed(1) }}</span>
      </div>
    </el-card>
  </div>
</template>
